...
export default function FeaturedMentors({ lang = 'en' }: { lang?: string }) {
  const isAr = lang === 'ar';
  const mentorsData = isAr ? mentorsAr : mentorsEn;
    ...
    <section className="relative py-28 px-6 lg:px-10 bg-surface/50 gradient-mesh">
      <div className="max-w-[1280px] mx-auto">
        <div className={`flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6 ${
          ...
          <div>
            ...
            <h2 className="text-[36px] md:text-[44px] font-bold tracking-tight text-navy-base leading-tight">
              {isAr ? (
                <>
                  {localCopy.titlePre}{' '}
                  <span className="text-gradient-purple">{localCopy.titleHighlight}</span>
                </>
              ) : (
                <>
                  {localCopy.titlePre}{' '}
          ...
          >
            {localCopy.viewAll}
            <ArrowRightIcon size={16} className={`transition-transform ${
              ...
              >
                ...
                <div className="relative h-[200px] bg-surface overflow-hidden">
                  ...
                  {mentor.current_mentees < 3 && (
                    <div className={`absolute top-3 ${isAr ? 'left-3' : 'right-3'} flex items-center gap-1.5 glass px-2.5 py-1 rounded-full`}>
                      <span className="w-1.5 h-1.5 rounded-full bg-green" />
                      <span className="text-[11px] font-semibold text-navy-base">{localCopy.available}</span>
                    </div>
                  )}
                </div>

                {/* Details */}
                <div className="p-5 flex-1 flex flex-col justify-between" style={{ textAlign: isAr ? 'right' : 'left' }}>
                  <div>
